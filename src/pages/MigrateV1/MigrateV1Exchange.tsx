...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WGLMR } from '@dctdao/sdk'
    ...
    <>
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(WGLMR[chainId]) ? 'WGLMR' : token.symbol}/ETH
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(WGLMR[chainId]) ? 'WGLMR' : token.symbol}:
        </Text>
        <RowFixed>
      ...
      <RowBetween mb="1rem">
        ...
        <RowFixed>
          ...
          <CurrencyLogo size="20px" style={{ marginLeft: '8px' }} currency={Currency.GLIMMER} />
        </RowFixed>
      </RowBetween>
    </>
  )
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const [v2PairState, v2Pair] = usePair(chainId ? WGLMR[chainId] : undefined, token)
  const isFirstLiquidityProvider: boolean = v2PairState === PairState.NOT_EXISTS

  const v2SpotPrice = chainId && v2Pair ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(WGLMR[chainId])) : undefined
  ...
  return (
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        ) : validatedAddress && chainId && token?.equals(WGLMR[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because Uniswap V2 uses WGLMR under the hood, your Uniswap V1 WGLMR/ETH liquidity cannot be migrated. You
