...
import { CurrencyAmount, GLIMMER, SwapParameters, Token, Trade, TradeOptions, TradeType } from '@dctdao/sdk'
...
export default function v1SwapArguments(trade: Trade, options: Omit<TradeOptions, 'feeOnTransfer'>): SwapParameters {
  ...
  const inputETH = trade.inputAmount.currency === GLIMMER
  const outputETH = trade.outputAmount.currency === GLIMMER
  if (inputETH && outputETH) throw new Error('GLIMMER to GLIMMER')
