...
export class MockV1Pair extends Pair {
  constructor(etherAmount: BigintIsh, tokenAmount: TokenAmount) {
    super(tokenAmount, new TokenAmount(WGLMR[tokenAmount.token.chainId], etherAmount))
  }
}
...
function useMockV1Pair(inputCurrency?: Currency): MockV1Pair | undefined {
  ...
  const isWGLMR = Boolean(token && token.equals(WGLMR[token.chainId]))
  const v1PairAddress = useV1ExchangeAddress(isWGLMR ? undefined : token?.address)
...
export function useV1Trade(
  ...
  const inputIsETH = inputCurrency === GLIMMER
  const outputIsETH = outputCurrency === GLIMMER
